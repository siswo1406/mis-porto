...
export default function AddUserModal({ isOpen, onClose, regions = [], units = [], jabatans = [] }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] overflow-y-auto">
            {/* Backdrop */}
            <div className="fixed inset-0 bg-slate-900/75 dark:bg-slate-950/80 backdrop-blur-sm transition-opacity" onClick={closeModal}></div>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0 relative z-10">
                <div className="relative transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl border border-slate-200 dark:border-slate-800">
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="bg-slate-50 dark:bg-slate-900/50 px-6 py-6 max-h-[60vh] overflow-y-auto">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 [&_label]:block [&_label]:text-sm [&_label]:font-medium [&_label]:text-slate-700 dark:[&_label]:text-slate-300 [&_input]:mt-1 [&_input]:block [&_input]:w-full [&_input]:rounded-lg [&_input]:border-slate-300 dark:[&_input]:border-slate-700 [&_input]:bg-white dark:[&_input]:bg-slate-800 [&_input]:text-slate-900 dark:[&_input]:text-slate-100 [&_input]:shadow-sm focus:[&_input]:border-blue-500 focus:[&_input]:ring-blue-500 sm:[&_input]:text-sm transition-colors [&_select]:mt-1 [&_select]:block [&_select]:w-full [&_select]:rounded-lg [&_select]:border-slate-300 dark:[&_select]:border-slate-700 [&_select]:bg-white dark:[&_select]:bg-slate-800 [&_select]:text-slate-900 dark:[&_select]:text-slate-100 [&_select]:shadow-sm focus:[&_select]:border-blue-500 focus:[&_select]:ring-blue-500 sm:[&_select]:text-sm">
                                ...
                                <div>
                                    ...
                                    <select value={data.roles} onChange={e => setData('roles', e.target.value)}>
                        ...
                        <div className="bg-white dark:bg-slate-900 px-4 py-4 sm:flex sm:flex-row-reverse sm:px-6 rounded-b-2xl border-t border-slate-100 dark:border-slate-800 transition-colors">
                            <button
                                type="submit"
                                disabled={processing}
                            ...
                            <button
                                ...
                                onClick={closeModal}
                                disabled={processing}
                                className="mt-3 inline-flex w-full justify-center rounded-lg bg-white dark:bg-slate-800 px-5 py-2.5 text-sm font-semibold text-slate-900 dark:text-slate-200 shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 sm:mt-0 sm:w-auto transition-colors"
                            >
                                Batal
                            </button>
                        </div>
